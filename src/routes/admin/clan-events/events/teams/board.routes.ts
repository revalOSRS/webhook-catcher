...
/**
 * Types for board responses (camelCase for API consistency)
 */
...
router.post('/line-buffs', async (req: Request, res: Response) => {
	try {
		...
		const existing = await query(
			...
			[board.id, lineType, lineType === 'column' ? lineIdentifier.toUpperCase() : lineIdentifier, buffDebuffId]
		);
		if (existing.length > 0) {
			return res.status(409).json({
				success: false,
...
router.post('/tiles/:tileId/complete', async (req: Request, res: Response) => {
	try {
		...
		const createEmptyProgressMetadata = () => {
			const reqProgress: Record<string, any> = {};
		...
		const existingProgress = await query(
			'SELECT id, progress_metadata FROM bingo_tile_progress WHERE board_tile_id = $1 LIMIT 1',
			[tileId]
		);
...
router.put('/bulk', async (req: Request, res: Response) => {
	try {
		...
		// Get or create board
		let boards = await query(
			'SELECT id, metadata, columns, rows FROM bingo_boards WHERE event_id = $1 AND team_id = $2',
			[eventId, teamId]
		);
		...
		if (tiles && tiles.length > 0) {
			for (const tile of tiles) {
				...
				if (progress) {
					const existingProgress = await query(
						'SELECT id, progress_metadata FROM bingo_tile_progress WHERE board_tile_id = $1',
						[currentBoardTileId]
					);
