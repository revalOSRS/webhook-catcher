...
import { BingoTilesEntity, BingoTile, BingoTileDifficulty } from '../../../../modules/events/bingo/entities/bingo-tiles.entity.js';
 ...
 */
const mapTileToResponse = (tile: BingoTile) => ({
	id: tile.id,
	task: tile.task,
...
router.get('/', async (req: Request, res: Response) => {
	try {
		...
		if (search) {
			// Use search method
			tiles = await tilesEntity.search(search as string);
		} else if (category) {
			// Filter by category
			tiles = await tilesEntity.findByCategory(category as any);
		} else if (difficulty) {
			// Filter by difficulty
			tiles = await tilesEntity.findByDifficulty(difficulty as BingoTileDifficulty);
		} else {
			// Get all
