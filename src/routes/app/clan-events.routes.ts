...
async function getMemberFromHeaders(req: Request): Promise<any | null> {
	...
	const members = await query(
		'SELECT id, discord_id, member_code FROM members WHERE discord_id = $1 AND member_code = $2 AND is_active = true',
		[discordId, code]
	);
...
router.get('/', async (req: Request, res: Response) => {
	try {
		...
		let participatingEventIds: string[] = [];
		if (member) {
			...
			`, [member.id]);
			participatingEventIds = participating.map((p: any) => p.event_id);
		}
...
router.get('/:eventId', async (req: Request, res: Response) => {
	try {
		...
		if (boards.length > 0) {
			...
			const tilesWithProgress: BoardTileWithProgress[] = await Promise.all(
				tiles.map(async (tile: any) => {
					...
					if (showTileBuffs) {
						...
						tileEffects = effects.filter((e: any) => e.is_active === true);
					}
