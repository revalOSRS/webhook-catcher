...
export class WiseOldManService {
   ...
   */
  static async searchPlayer(username: string) {
    try {
   ...
   */
  static async updatePlayer(username: string) {
    try {
   ...
   */
  static async getPlayerSnapshots(username: string, limit: number = 10) {
    try {
   ...
   */
  static async getPlayerAchievements(username: string, limit: number = 20) {
    try {
   ...
   * Get player records
   */
  static async getPlayerRecords(username: string, period: string = 'week', metric?: string) {
    try {
   ...
   */
  static async getPlayerGroups(username: string) {
    try {
   ...
   */
  static async getComprehensivePlayerData(username: string) {
    try {
   ...
   */
  private static async retryFailedMembersBackground(groupId: number) {
   ...
   */
  static async getGroupStatistics(groupId: number) {
    try {
      ...
      if (this.groupStatisticsCache && !shouldRefresh) {
        ...
        if (this.failedMembersCache && this.failedMembersCache.members.length > 0) {
          ...
          this.retryFailedMembersBackground(groupId).catch(err =>
      ...
      if (this.failedMembersCache && this.failedMembersCache.members.length > 0) {
        ...
        const retryResults = await Promise.allSettled(
          this.failedMembersCache.members.map(member => client.players.getPlayerDetailsById(member.player.id))
        )
