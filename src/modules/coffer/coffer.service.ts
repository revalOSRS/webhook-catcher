...
import type { CofferMovement } from './coffer-movements.entity.js'
...
export class CofferService {
  private static readonly movementsEntity = new CofferMovementsEntity()
   ...
   */
  static async getRecentMovements(limit: number = 10): Promise<Array<CofferMovement & {
   ...
   */
  static async getMovementsByType(type: string, limit: number = 50): Promise<Array<CofferMovement & {
    playerUsername?: string
    eventName?: string
  }>> {
   ...
   */
  static async getMovementsByDateRange(startDate: Date, endDate: Date): Promise<Array<CofferMovement & {
    playerUsername?: string
    eventName?: string
  }>> {
   ...
   */
  static async getCofferStats(): Promise<{
   ...
   */
  static async updateBalanceManually(newBalance: number, updatedBy: string): Promise<number> {
    const currentBalance = await CofferBalanceEntity.getCurrentBalance()
   ...
   */
  static async getMovementsByPlayer(discordId: string, limit: number = 20): Promise<Array<CofferMovement & {
    eventName?: string
  }>> {
   ...
   */
  static async getPlayerDonationTotal(discordId: string): Promise<number> {
