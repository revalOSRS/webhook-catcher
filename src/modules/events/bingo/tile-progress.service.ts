...
export class TileProgressService {
   ...
   */
  private getTileBasePoints = async (tileId: string): Promise<number> => {
    const result = await query<{ points: number }>(
      'SELECT points FROM bingo_tiles WHERE id = $1',
      [tileId]
    );
   ...
   */
  private awardPointsToTeam = async (teamId: string, points: number): Promise<void> => {
    await query(
  ...
  private sendNotificationIfNeeded = async (
    ...
    try {
      ...
      if (event.osrsAccountId) {
        const accounts = await query<{ osrsNickname: string }>(
