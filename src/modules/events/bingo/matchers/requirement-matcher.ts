...
const matchesItemDrop = (event: UnifiedGameEvent, requirement: ItemDropRequirement): boolean => {
  ...
  if (requirement.sources && requirement.sources.length > 0) {
    ...
    if (!eventSource) return false // No source in event, can't match
    
    const matchesSource = requirement.sources.some(
      allowedSource => eventSource === allowedSource.toLowerCase()
    )
    if (!matchesSource) return false // Source doesn't match any allowed source
  }
