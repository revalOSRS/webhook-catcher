...
/**
 * Killcount Data Interface
 */
export interface Killcount extends BaseEntityData {
  id?: number
 ...
 */
export class KillcountsEntity extends BaseEntity<Killcount> {
  protected tableName = 'osrs_account_killcounts'
  protected primaryKey = 'id'
  protected camelCaseFields = ['osrsAccountId', 'bossName', 'killCount', 'lastKilledAt', 'createdAt', 'updatedAt']
   ...
   */
  async getByAccountId(accountId: number): Promise<Killcount[]> {
    return this.findAll({
   ...
   */
  async getByBossAndAccount(accountId: number, bossName: string): Promise<Killcount | null> {
    return this.findOne({
   ...
   */
  async upsertKillcount(accountId: number, bossName: string, killCount: number): Promise<Killcount> {
   ...
   */
  async incrementKillcount(accountId: number, bossName: string): Promise<Killcount> {
    const existing = await this.getByBossAndAccount(accountId, bossName)
   ...
   */
  async getTopKillcounts(bossName: string, limit: number = 10): Promise<Killcount[]> {
    const sql = `
   ...
   */
  async getBossStats(): Promise<Array<{ bossName: string; totalKills: number; uniquePlayers: number; averageKills: number }>> {
    const sql = `
      SELECT
        boss_name,
