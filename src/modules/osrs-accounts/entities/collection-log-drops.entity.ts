...
/**
 * Collection Log Drop Data Interface
 */
export interface CollectionLogDrop extends BaseEntityData {
  id?: number
  osrsAccountId: number
  collectionLogItemId: number
  obtainedAt?: Date
  quantity?: number
}
 ...
 */
export class CollectionLogDropsEntity extends BaseEntity<CollectionLogDrop> {
  protected tableName = 'osrs_account_collection_log_drops'
  protected primaryKey = 'id'
  protected camelCaseFields = ['osrsAccountId', 'collectionLogItemId', 'obtainedAt']
   ...
   */
  async getDropsWithDetails(accountId: number): Promise<Array<CollectionLogDrop & {
    itemName?: string
    category?: string
    subcategory?: string
    rarity?: string
   ...
   */
  async hasObtained(accountId: number, itemId: number): Promise<boolean> {
