...
const validateChatRequirement = (req: ChatRequirement): string[] => {
  const errors: string[] = [];
...
const validatePuzzleRequirement = (req: PuzzleRequirement): string[] => {
  const errors: string[] = [];
  ...
  if (!req.hiddenRequirement || typeof req.hiddenRequirement !== 'object') {
    ...
    if ((req.hiddenRequirement as any).type === 'PUZZLE') {
 ...
 */
const validateSimplifiedRequirementInternal = (requirement: any): string[] => {
  const errors: string[] = [];
