...
// ===== Point Categories =====
export var PointCategory;
(function (PointCategory) {
    PointCategory["QUESTS"] = "quests";
    PointCategory["ACHIEVEMENT_DIARIES"] = "achievement_diaries";
    PointCategory["COMBAT_ACHIEVEMENTS"] = "combat_achievements";
    PointCategory["COLLECTION_LOG"] = "collection_log";
    PointCategory["BOSS_KILLS"] = "boss_kills";
    PointCategory["SKILLS"] = "skills";
})(PointCategory || (PointCategory = {}));
// ===== Calculate Points from SYNC Event =====
export function calculatePointsFromSync(payload) {
    const breakdown = {
        total: 0,
        byCategory: {
...
function calculateQuestPoints(questPoints) {
    return questPoints * POINTS_CONFIG.questPoints.perPoint;
}
function calculateDiaryPoints(diaryProgress) {
    let totalPoints = 0;
    for (const progress of Object.values(diaryProgress)) {
        if (progress.easy)
            totalPoints += POINTS_CONFIG.achievementDiaries.easy;
        if (progress.medium)
            totalPoints += POINTS_CONFIG.achievementDiaries.medium;
        if (progress.hard)
            totalPoints += POINTS_CONFIG.achievementDiaries.hard;
        if (progress.elite)
            totalPoints += POINTS_CONFIG.achievementDiaries.elite;
    }
    return totalPoints;
}
function calculateCollectionLogPoints(obtainedItems) {
 ...
 */
function calculateBossKillPoints(collectionLog) {
    let totalPoints = 0;
 ...
 */
export function calculatePointsDifference(newBreakdown, oldBreakdown) {
    return {
        total: newBreakdown.total - oldBreakdown.total,
        byCategory: {
            [PointCategory.QUESTS]: newBreakdown.byCategory[PointCategory.QUESTS] - oldBreakdown.byCategory[PointCategory.QUESTS],
            [PointCategory.ACHIEVEMENT_DIARIES]: newBreakdown.byCategory[PointCategory.ACHIEVEMENT_DIARIES] - oldBreakdown.byCategory[PointCategory.ACHIEVEMENT_DIARIES],
            [PointCategory.COMBAT_ACHIEVEMENTS]: newBreakdown.byCategory[PointCategory.COMBAT_ACHIEVEMENTS] - oldBreakdown.byCategory[PointCategory.COMBAT_ACHIEVEMENTS],
            [PointCategory.COLLECTION_LOG]: newBreakdown.byCategory[PointCategory.COLLECTION_LOG] - oldBreakdown.byCategory[PointCategory.COLLECTION_LOG],
            [PointCategory.BOSS_KILLS]: newBreakdown.byCategory[PointCategory.BOSS_KILLS] - oldBreakdown.byCategory[PointCategory.BOSS_KILLS],
            [PointCategory.SKILLS]: newBreakdown.byCategory[PointCategory.SKILLS] - oldBreakdown.byCategory[PointCategory.SKILLS]
        }
    };
}
/**
