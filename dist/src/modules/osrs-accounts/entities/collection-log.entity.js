...
export class CollectionLogEntity extends BaseEntity {
    tableName = 'osrs_account_collection_log';
    primaryKey = 'id';
    camelCaseFields = ['osrsAccountId', 'itemName', 'obtainedAt', 'createdAt', 'updatedAt'];
    /**
     ...
     */
    async getByCategory(accountId, category, subcategory) {
        let where = {
     ...
     */
    async markObtained(accountId, category, subcategory, itemName, quantity = 1) {
     ...
     */
    async bulkUpdateFromSync(accountId, collectionLogData) {
     ...
     */
    async getRecentlyObtained(accountId, limit = 10) {
        return this.findAll({
            where: {
                osrsAccountId: accountId,
                obtained: true
