 ...
 * Basic CRUD operations for OSRS accounts
 */
export class OsrsAccountsEntity extends BaseEntity {
    tableName = 'osrs_accounts';
    primaryKey = 'id';
    camelCaseFields = ['discordId', 'osrsNickname', 'dinkHash', 'womPlayerId', 'womRank', 'isPrimary', 'lastSyncedAt', 'createdAt', 'updatedAt'];
    /**
     ...
     */
    async findByOsrsNickname(osrsNickname) {
     ...
     */
    async getHighestEfficiency(discordId) {
        const sql = `
     ...
     */
    async getAllWithEfficiency(discordId) {
        const sql = `
      SELECT *, (ehp + ehb) as total_efficiency
      FROM ${this.tableName}
      WHERE discord_id = $1
     ...
     */
    async updateSyncTimestamp(accountId) {
        try {
