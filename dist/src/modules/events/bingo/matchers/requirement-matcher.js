...
export const matchesRequirement = (event, requirements) => {
    ...
    if (requirements.tiers && requirements.tiers.length > 0) {
        matchesTier = requirements.tiers.some(tier => matchesSimplifiedRequirement(event, tier.requirement));
    }
    ...
    if (requirements.requirements && requirements.requirements.length > 0) {
        if (requirements.matchType === BingoTileMatchType.ALL) {
            ...
            matchesBase = requirements.requirements.some(req => matchesSimplifiedRequirement(event, req));
        }
        else {
 ...
 */
const matchesChat = (event, requirement) => {
    if (event.eventType !== UnifiedEventType.CHAT)
        return false;
    ...
    if (!eventSource || !ALLOWED_CHAT_SOURCES.includes(eventSource)) {
        return false;
    }
 ...
 */
const matchesPuzzle = (event, requirement) => {
