 ...
 */
async function processUnifiedEvent(event) {
 ...
 */
async function getActiveBoardTilesForPlayer(event) {
 ...
 */
async function updateTileProgress(event, tile) {
 ...
 */
async function calculateRequirementProgress(event, requirement, existing, eventStartDate, tier) {
    const req = 'requirement' in requirement ? requirement.requirement : requirement;
    switch (req.type) {
        case 'ITEM_DROP':
            return calculateItemDropProgress(event, req, existing);
        case 'PET':
            return calculatePetProgress(event, req, existing);
        case 'VALUE_DROP':
            return calculateValueDropProgress(event, req, existing);
        case 'SPEEDRUN':
            return calculateSpeedrunProgress(event, tier || req, existing);
        case 'BA_GAMBLES':
            return calculateBaGamblesProgress(event, req, existing);
        case 'EXPERIENCE':
            return await calculateExperienceProgress(event, req, existing, eventStartDate);
        default:
            return {
                progressValue: existing?.progressValue || 0,
                metadata: existing?.metadata || {},
 ...
 */
async function getExistingProgress(boardTileId, osrsAccountId) {
 ...
 * Insert new progress
 */
async function insertProgress(boardTileId, teamId, osrsAccountId, progressUpdate) {
    await query(`
 ...
 */
async function updateProgress(boardTileId, osrsAccountId, progressUpdate) {
    await query(`
 ...
 */
async function markTileCompleted(boardTileId, osrsAccountId, completionType) {
