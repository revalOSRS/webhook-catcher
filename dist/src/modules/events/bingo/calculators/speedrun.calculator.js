 ...
 */
export const calculateSpeedrunProgress = (event, requirement, existing, memberId, osrsAccountId, playerName) => {
    // Check if this is a tiered requirement
    if ('tier' in requirement && 'points' in requirement) {
        return calculateTieredSpeedrunProgress(event, requirement, existing, memberId, osrsAccountId, playerName);
    }
    return calculateSimpleSpeedrunProgress(event, requirement, existing, memberId, osrsAccountId, playerName);
};
/**
 * Calculate simple (non-tiered) speedrun progress.
 */
const calculateSimpleSpeedrunProgress = (event, requirement, existing, memberId, osrsAccountId, playerName) => {
 ...
 */
const calculateTieredSpeedrunProgress = (event, tieredReq, existing, memberId, osrsAccountId, playerName) => {
    const speedrunData = event.data;
    const currentTime = speedrunData.timeSeconds;
    ...
    const completedTiers = existingMetadata?.completedTiers
 ...
 */
const getOrInitPlayerContributions = (existingMetadata, osrsAccountId, playerName, memberId, initialTime) => {
    const contributions = existingMetadata?.playerContributions ? [...existingMetadata.playerContributions] : [];
