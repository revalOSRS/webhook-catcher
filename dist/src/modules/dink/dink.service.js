...
export class DinkService {
     ...
     */
    static getDeathDescription(playerName, killerName) {
        // Estonian vowels
     ...
     */
    static formatDinkEvent(payloadData) {
        const eventMap = {
            'COLLECTION': {
                icon: '📖',
     ...
     */
    static async createDiscordPayload(fields, imageBuffer, imageFilename) {
     ...
     */
    static getSendFunction(eventType) {
        switch (eventType) {
            case 'LOOT':
            case 'COLLECTION':
            case 'CHAT':
     ...
     */
    static async captureLoginXpSnapshot(playerName, payloadData) {
    ...
    `, [account.id]);
        ...
        for (const { eventId } of activeEvents) {
            try {
                const snapshot = await BingoXpSnapshotsService.captureLoginSnapshot(eventId, account.id, skills.experience, // e.g., { "Attack": 7285226, "Strength": 14602484, ... }
