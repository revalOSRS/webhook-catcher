...
router.post('/', async (req, res) => {
    try {
        ...
        if (team_id) {
            const teamCheck = await query('SELECT id FROM event_teams WHERE id = $1 AND event_id = $2', [team_id, event_id]);
...
router.post('/:id/tiles', async (req, res) => {
    try {
        ...
        const positionCheck = await query('SELECT id FROM bingo_board_tiles WHERE board_id = $1 AND position = $2', [id, position]);
 ...
 * This endpoint is kept for backwards compatibility
 */
router.delete('/:boardId/tiles/:tileId', async (req, res) => {
    try {
 ...
 * This endpoint is kept for backwards compatibility
 */
router.patch('/:boardId/tiles/:tileId', async (req, res) => {
    try {
        const { boardId, tileId } = req.params;
        const updates = req.body;
        // Check if board tile exists
        const existing = await query('SELECT id FROM bingo_board_tiles WHERE board_id = $1 AND id = $2', [boardId, tileId]);
        if (existing.length === 0) {
            return res.status(404).json({
                success: false,
                error: 'Board tile not found'
        ...
        for (const [key, value] of Object.entries(updates)) {
            if (allowedFields.includes(key)) {
                ...
                if (key === 'position' && value !== existing[0].position) {
                    const posCheck = await query('SELECT id FROM bingo_board_tiles WHERE board_id = $1 AND position = $2 AND id != $3', [boardId, value, tileId]);
