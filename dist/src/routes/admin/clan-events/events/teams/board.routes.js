        ...
        if (tiles && tiles.length > 0) {
            for (const tile of tiles) {
                ...
                if (progress) {
                    ...
                    const newTierNumbers = newTiers.map((t) => t.tier);
                    // Find newly completed tiers
                    const newlyCompletedTiers = newTierNumbers.filter((t) => !existingTierNumbers.includes(t));
                    if (existingProgress.length > 0) {
                    ...
                    if (newlyCompletedTiers.length > 0) {
                        ...
                        if (tileReqs.length > 0 && tileReqs[0].requirements?.tiers) {
                            ...
                            for (const tierNum of newlyCompletedTiers) {
                                const tierDef = tileReqs[0].requirements.tiers.find((t) => t.tier === tierNum);
                                if (tierDef?.points) {
                                    tierPointsToAdd += tierDef.points;
                                }
                            }
                            if (tierPointsToAdd > 0) {
                                await query('UPDATE event_teams SET score = score + $1, updated_at = NOW() WHERE id = $2', [tierPointsToAdd, teamId]);
