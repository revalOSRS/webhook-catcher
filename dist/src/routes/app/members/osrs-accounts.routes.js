...
router.get('/:memberId/osrs-accounts/:osrsAccountId', requireMemberAuth, async (req, res) => {
    ...
    `);
        ...
        const playerSnapshot = await db.queryOne(`
      SELECT 
    ...
    `, [osrsAccount.wom_player_id, latestClanSnapshot.id]);
        ...
        const [skills, computed, collectionLogItems, collectionLogDrops, combatAchievements, diaryCompletions, killCounts, questsData] = await Promise.all([
            // WOM Skills Snapshot
            db.query(`
      ...
      `, [osrsAccount.id])
        ...
        const collectionLogByCategory = collectionLogItems.reduce((acc, item) => {
        ...
        const uniqueItemIds = new Set(collectionLogItems.map((item) => item.item_id));
        const totalUniqueItems = uniqueItemIds.size;
        // Group diary completions by area
        const diariesByArea = diaryCompletions.reduce((acc, diary) => {
        ...
        const combatAchievementsByTier = combatAchievements.reduce((acc, ca) => {
            if (!acc[ca.tier]) {
                acc[ca.tier] = [];
            }
            acc[ca.tier].push({
            ...
            data: {
                ...
                achievements: {
                    ...
                    diaries: {
                        ...
                        summary: {
                            easy: diaryCompletions.filter((d) => d.tier === 'easy').length,
                            medium: diaryCompletions.filter((d) => d.tier === 'medium').length,
                            hard: diaryCompletions.filter((d) => d.tier === 'hard').length,
                            elite: diaryCompletions.filter((d) => d.tier === 'elite').length
                        }
                    },
                    combat_achievements: {
                        ...
                        summary: {
                            easy: combatAchievements.filter((ca) => ca.tier === 'Easy').length,
                            medium: combatAchievements.filter((ca) => ca.tier === 'Medium').length,
                            hard: combatAchievements.filter((ca) => ca.tier === 'Hard').length,
                            elite: combatAchievements.filter((ca) => ca.tier === 'Elite').length,
                            master: combatAchievements.filter((ca) => ca.tier === 'Master').length,
                            grandmaster: combatAchievements.filter((ca) => ca.tier === 'Grandmaster').length
                        }
                    }
                },
                // Collection Log Data
                collection_log: {
                    ...
                    total_unique_sources: Object.keys(collectionLogByCategory).reduce((count, category) => {
                    ...
                    by_category: Object.keys(collectionLogByCategory).map(category => {
                        ...
                        const categoryItems = Object.values(collectionLogByCategory[category]).flat();
                        const uniqueItemIdsInCategory = new Set(categoryItems.map((item) => item.item_id));
                        return {
                            category,
                    ...
                    recent_drops: collectionLogDrops.map((drop) => ({
                ...
                kill_counts: {
                    ...
                    total_kills: killCounts.reduce((sum, kc) => sum + (kc.kill_count || 0), 0),
                    by_category: killCounts.reduce((acc, kc) => {
                        if (!acc[kc.category]) {
