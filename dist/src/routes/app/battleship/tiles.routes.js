...
router.post('/:tileId/progress', async (req, res) => {
    try {
        const { tileId } = req.params;
        const { discord_id, progress_amount, progress_percentage, contribution_type, current_best_value, proof_url, notes } = req.body;
...
router.post('/:tileId/complete', async (req, res) => {
    try {
        const { tileId } = req.params;
        const { completed_by_discord_id, contributors, bonus_tier_achieved, completion_value, total_points_awarded, proof_url } = req.body;
